					...
					<form action="" id="manage-menu">
						<div class="card">
							...
							<div class="card-body">
								...
								<input type="hidden" name="user_id" value="<?php echo $_SESSION['rlogin_id'] ?>" class="form-control">
							</div>
							<div class="card-footer">
								<div class="row">
									<div class="col-md-12">
										<button type="button" class="btn btn-sm btn-primary col-sm-4 offset-md-3">save</button>
										<button type="button" class="btn btn-sm btn-secondary col-sm-4" onclick="$('#manage-menu').get(0).reset()">cancel</button>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="card-body">
							<table class="table table-bordered table-hover">
								...
								<tbody>
									<?php 
									$i = 1;
									$cats = $conn->query("SELECT * FROM product_list WHERE user_id = '".$_SESSION['rlogin_id']."' order by id asc");
									...
									<tr>
										...
										<td class="text-center">
											<button class="btn btn-sm btn-primary edit_menu m-2" type="button" data-id="<?php echo $row['id'] ?>" data-name="<?php echo $row['name'] ?>" data-status="<?php echo $row['status'] ?>" data-description="<?php echo $row['description'] ?>" data-price="<?php echo $row['price'] ?>" data-img_path="<?php echo $row['img_path'] ?>">Edit</button>
...
function delete_menu($id){
	start_load()
	$.ajax({
		url:'../admin/ajax.php?action=delete_menu',
		method:'POST',
		data:{id:$id},
		success:function(resp){
			if(resp==1){
