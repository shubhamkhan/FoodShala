...
Class Action {
	...
	function login(){
		extract($_POST);
		$qry = $this->db->query("SELECT * FROM users where email = '".$email."' and password = '".md5($password)."' and type = '".$type."' ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
				if($key != 'passwors' && !is_numeric($key))
					$_SESSION['alogin_'.$key] = $value;
			}
			return 1;
		}else{
			return 3;
	...
	function signup3(){
		...
		$chk = $this->db->query("SELECT * FROM system_settings where email = '$email' ")->num_rows;
		if($chk > 0){
			return 2;
			exit;
		}
			$save = $this->db->query("INSERT INTO system_settings set ".$data);
			$save = $this->db->query("INSERT INTO users set ".$data2);
		if($save){
			$login = $this->login3();
			return 1;
		}
	...
	function save_settings(){
		...
		$chk = $this->db->query("SELECT * FROM system_settings WHERE id = '".$wid."'");
		if($chk->num_rows > 0){
			$save = $this->db->query("UPDATE system_settings set ".$data." where id =".$chk->fetch_array()['id']);
		}else{
			$save = $this->db->query("INSERT INTO system_settings set '".$data."' WHERE id = '".$wid."'");
		}
		if($save){
		$query = $this->db->query("SELECT * FROM system_settings WHERE id = '".$wid."'")->fetch_array();
